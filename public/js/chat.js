...
$(function () {
    ...
    socketListener = function () {
        socket.on('connect', function () {
            socket.on("system message", function (data) {
                appendHtml(rHtml(data.msg, '', '', '0', ''));
                refreshOnline(data.online);
            });

            socket.on('public message', function (data) {
                appendHtml(rHtml(data.msg, data.user, '', '1', ''));
            });

            socket.on('private message', function (data) {

                appendHtml(rHtml(data.msg, data.user, '我', '2', ''));
            });

            socket.on('err', function (data) {
                appendHtml(rHtml(data.msg, '', '', '3', ''));
            });
            ...
            socket.on('record', function (data) {
                ...
                for (var i = 0; i < records.length; i++) {
                    ...
                    htmls += rHtml(record[0], record[1], '', '4', record[2]);
                }
                appendHtml(htmls);
    ...
    //t 用于聊天记录 时间
    var rHtml = function (msg, from, to, type, t) {
